...
namespace Randomizer.Randomizers.Game3.Misc
{
    class Cutscene
    {
        ...
        {
            if (!export.IsDefaultObject && export.ClassName == "SeqAct_Interp" && KismetHelper.GetComment(export) is string str && str.StartsWith("Auto-generated"))
            {
                return true;
            }
            return false;
        ...
        public static bool ShuffleCutscenePawns2(GameTarget target, ExportEntry export, RandomizationOption option)
        {
            ...
            var cutsceneName = KismetHelper.GetParentSequence(export).ObjectName;
