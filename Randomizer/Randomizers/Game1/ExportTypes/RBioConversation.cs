...
namespace Randomizer.Randomizers.Game1.ExportTypes
{
    ...
    class RBioConversation
    {
        ...
        public static bool RandomizeExport(GameTarget target, ExportEntry export, RandomizationOption option)
        {
            ...
            var varLinks = SeqTools.GetVariableLinksOfNode(export);
            var allOptions = varLinks.SelectMany(x => x.LinkedNodes).ToList();
            allOptions.Shuffle();

            foreach(var v in varLinks)
            {
                if (v.LinkedNodes.Any())
                {
                    v.LinkedNodes.Clear();
                    v.LinkedNodes.Add(allOptions.PullFirstItem());
                }
            }

            SeqTools.WriteVariableLinksToNode(export, varLinks);
            return true;
        }
